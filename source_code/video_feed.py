import numpy as np
import cv2 as cv2
import matplotlib.pyplot as plt

# "Displays the velocity "
def display_velocity(curr_velocity, curr_frame):
    """
    param: curr_velocity - scalar
           curr_frame = math.shape --> Current frame
    """
    pass
    return None

def extract_blob_features(image, ddepth, ksize, bsize):
    blurred = cv2.GaussianBlur(image, (bsize, bsize), 0)
    LoG = cv2.Laplacian(src=blurred, ddepth=ddepth, ksize=ksize)
    print(image.shape)
    #LoG += 128

    #print(type(LoG))
    print(LoG.shape)
    #grad = np.convolve(LoG, image)
    #thresh, imThresh = cv2.threshold(src=image, thresh=0, maxval=255, type=cv2.THRESH_BINARY+cv2.THRESH_OTSU)

    return LoG

def detect_keypoints(image):
    pass

    return None

# "Bounding-Box"
def bounding_box(object, threshold=0.5):
    """
    param: object - object-detection
           threshold - scalar, determines the strictness of tracking.
    """

    pass
    return None

# "Measures the velocity"
def measure_velocity(frame_1, frame_2, method):
    """
    param: frame_1 - math.shape
           frame_2 - math.shape
    """

    pass
    return None

# "Necessary preprocessing of video-frames"
def preprocess_frames(frame_1, frame_2, frame_list):
    """
    param: frame_1 - math.shape
           frame_2 - math.shape
           frame_list - np.array()
    """

    pass
    return None

"Getting camera-feed"
def open_camera(cam_index, width, height,delta):
    cap = cv2.VideoCapture(cam_index)
    curr_frame = None
    ref_frame = None
    while(True):
        ret, frame = cap.read()
        frame = cv2.resize(frame, (width+(3*delta),height+(4*delta)))
        cv2.imshow('frame',frame)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    cap.release()
    cv2.destroyAllWindows()


def main():
    open_camera(cam_index=0, width=640, height=480, delta=0)
    pass

if __name__ == '__main__':
    main()
